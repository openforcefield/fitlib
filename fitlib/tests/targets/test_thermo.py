...
from fitlib.targets.thermo import (
    DataEntry,
    SimulationConfig,
    SimulationKey,
...
def test_create_dataset(mock_density_pure, mock_density_binary):
    ...
    entries = list(fitlib.utils.dataset.iter_dataset(dataset))
...
def test_plan_simulations(
    ...
    custom_config = {
        "bulk": SimulationConfig(
            max_mols=max_mols,
            gen_coords=fitlib.mm.GenerateCoordsConfig(),
            equilibrate=[fitlib.mm.MinimizationConfig()],
            production=fitlib.mm.SimulationConfig(
                temperature=300 * openmm.unit.kelvin,
                pressure=None,
                n_steps=5000,
...
def test_simulation(tmp_cwd, mocker):
    ...
    mock_gen_coords = mocker.patch(
        "fitlib.mm.generate_system_coords",
        autospec=True,
        return_value=(coords, box_vectors),
    )
    mock_simulate = mocker.patch("fitlib.mm.simulate", autospec=True)

    spied_reporter = mocker.spy(fitlib.mm, "TensorReporter")
...
def test_compute_observables_reweighted(tmp_cwd, mocker):
    ...
    mock_reweight = mocker.patch(
        "fitlib.mm.reweight_ensemble_averages", autospec=True, return_value=mock_result
    )
...
def test_compute_observables_simulated(tmp_cwd, mocker):
    ...
    mocker.patch(
        "fitlib.mm.reweight_ensemble_averages",
        autospec=True,
        side_effect=fitlib.mm._ops.NotEnoughSamplesError(),
    )
    mock_simulate = mocker.patch("fitlib.targets.thermo._simulate", autospec=True)
    mock_compute = mocker.patch(
        "fitlib.mm.compute_ensemble_averages", autospec=True, return_value=mock_result
    )
...
def test_predict_hvap(mock_hvap, mocker):
    ...
    system_vacuum = fitlib.TensorSystem([topologies["CCCC"]], [1], False)
...
def test_predict_hmix(mock_hmix, mocker):
    ...
    system_0 = fitlib.TensorSystem([topologies["CCO"]], [n_mols], False)
    system_1 = fitlib.TensorSystem([topologies["CO"]], [n_mols], False)
...
def test_predict(tmp_cwd, mock_density_pure, mocker):
    ...
    mock_compute = mocker.patch(
        "fitlib.targets.thermo._compute_observables",
        autospec=True,
