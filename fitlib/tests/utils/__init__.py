...
def mock_convert_fn_with_deps(
    ...
    v_site_maps: list[fitlib.VSiteMap | None],
    dependencies: dict[
        str, tuple[fitlib.TensorPotential, list[fitlib.NonbondedParameterMap]]
    ],
) -> tuple[fitlib.TensorPotential, list[fitlib.NonbondedParameterMap]]:
    raise NotImplementedError()


def convert_lj_to_dexp(potential: fitlib.TensorPotential):
    potential.fn = fitlib.EnergyFn.VDW_DEXP
...
def topology_from_smiles(smiles: str) -> fitlib.TensorTopology:
...
def system_from_smiles(
...
) -> tuple[fitlib.TensorSystem, fitlib.TensorForceField]:
    ...
    return fitlib.TensorSystem(tensor_tops, n_copies, is_periodic=True), tensor_ff


def _parameter_key_to_idx(potential: fitlib.TensorPotential, key: str):
    return next(iter(i for i, k in enumerate(potential.parameter_keys) if k.id == key))


def system_with_exceptions() -> tuple[
    fitlib.TensorSystem, fitlib.TensorPotential, dict[str, LJParam]
]:
    ...
    for top in system.topologies:
        assignment_matrix = fitlib.utils.zeros_like(
...
def add_explicit_lb_exceptions(
    ...
    for top in system.topologies:
        ...
        padding = fitlib.utils.zeros_like(
