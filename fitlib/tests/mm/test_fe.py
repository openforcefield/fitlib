...
def load_systems(solute: str, solvent: str):
    ...
    ff, (top_solute, top_solvent) = fitlib.converters.convert_interchange(
...
def test_extract_pure_solvent(tmp_cwd, mocker):
    ...
    system = fitlib.TensorSystem([top_solute, top_solvent], [1, 10], True)
    xyz, box = fitlib.mm.generate_system_coords(system, ff)
    ...
    mocker.patch(
        "fitlib.mm._fe._load_samples",
        return_value=(system, None, None, None, None, xyz, box),
    )

    xyz_solv, _, _ = fitlib.mm._fe._extract_pure_solvent(
...
def test_fe_ops(tmp_cwd):
    ...
    fitlib.mm.generate_dg_solv_data(
    ...
    dg_comp = fitlib.mm.compute_dg_solv(top_solute, None, top_solvent, ff, output_dir)
    ...
    dg_rw, n_eff = fitlib.mm.reweight_dg_solv(
