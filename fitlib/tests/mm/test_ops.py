...
from fitlib.mm._reporters import _encoder


def _mock_potential(type_, parameters, attributes) -> fitlib.TensorPotential:
    return fitlib.TensorPotential(
        type_,
        f"{type_}-fn",
        parameters,
        [
...
def test_pack_unpack_force_field(mocker):
    ...
    force_field = fitlib.TensorForceField(
        ...
        fitlib.TensorVSites(v_site_keys, v_site_weights, v_site_parameters),
    )
...
def test_compute_observables(tmp_path, mock_argon_tensors, mock_argon_params):
    ...
    tensor_system = fitlib.TensorSystem([tensor_top], [2], False)
...
def test_compute_ensemble_averages(mocker, tmp_path, mock_argon_tensors):
    ...
    mock_compute_observables = mocker.patch(
        "fitlib.mm._ops._compute_observables",
        autospec=True,
...
def test_compute_dg_solv(mocker, tmp_path, mock_argon_tensors):
    ...
    mocker.patch(
        "fitlib.mm._fe.compute_dg_and_grads",
        side_effect=[
    ...
    dg = fitlib.mm.compute_dg_solv(tensor_top, None, tensor_top, tensor_ff, tmp_path)
...
def test_reweight_dg_solv(mocker, tmp_path, mock_argon_tensors):
    ...
    mocker.patch(
        "fitlib.mm._fe.reweight_dg_and_grads",
        side_effect=[
    ...
    dg, n_eff = fitlib.mm.reweight_dg_solv(
...
def test_reweight_dg_solv_error(mocker, tmp_path, mock_argon_tensors):
    ...
    with pytest.raises(fitlib.mm.NotEnoughSamplesError):
        fitlib.mm.reweight_dg_solv(
