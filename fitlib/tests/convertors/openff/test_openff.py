...
import fitlib
from fitlib.converters.openff._openff import (
    _CONVERTERS,
    _convert_topology,
    _Converter,
...
def test_convert_handler(ethanol, ethanol_interchange, mocker):
    ...
    v_site_maps = [
        fitlib.VSiteMap([v_site], {v_site: ethanol.n_atoms}, torch.tensor([[0]]))
    ]
...
def test_convert_topology(formaldehyde, mocker):
    ...
    v_sites = fitlib.VSiteMap([], {}, torch.tensor([]))
    constraints = fitlib.TensorConstraints(torch.tensor([1, 2]), torch.tensor([3.0]))
