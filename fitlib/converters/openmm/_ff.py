...
_CONVERTER_FUNCTIONS: dict[
    tuple[str, str],
    typing.Callable[
        [fitlib.TensorPotential, fitlib.TensorTopology, list[str]], ElementTree.Element
    ],
] = {}
...
def ffxml_converter(potential_type: str, energy_expression: str):
    ...
    The decorated function should take a `fitlib.TensorPotential`, and
    the associated `fitlib.ParameterMap` and list of atom types, and return a
...
def _get_param_col(pot: fitlib.TensorPotential, param_name: str) -> tuple[int, float]:
...
def _to_residue(
    top: fitlib.TensorTopology,
    ff: fitlib.TensorForceField,
    ...
    for v_site_idx, param_idx in enumerate(top.v_sites.parameter_idxs.tolist()):
        local_frame_coords = (
            fitlib.geometry.polar_to_cartesian_coords(
...
def default_valence_converter(
    pot: fitlib.TensorPotential,
    param_map: fitlib.ValenceParameterMap,
    atom_types: list[str],
...
@ffxml_converter(fitlib.PotentialType.VDW, fitlib.EnergyFn.VDW_LJ)
def convert_lj_force(
    pot: fitlib.TensorPotential,
    param_map: fitlib.NonbondedParameterMap,
    types: list[str],
) -> ElementTree.Element:
...
@ffxml_converter(fitlib.PotentialType.ELECTROSTATICS, fitlib.EnergyFn.COULOMB)
...
@ffxml_converter(fitlib.PotentialType.BONDS, fitlib.EnergyFn.BOND_HARMONIC)
def convert_bond_potential(
    pot: fitlib.TensorPotential, param_map: fitlib.ValenceParameterMap, types: list[str]
):
...
@ffxml_converter(fitlib.PotentialType.ANGLES, fitlib.EnergyFn.ANGLE_HARMONIC)
...
@ffxml_converter(fitlib.PotentialType.PROPER_TORSIONS, fitlib.EnergyFn.TORSION_COSINE)
@ffxml_converter(fitlib.PotentialType.IMPROPER_TORSIONS, fitlib.EnergyFn.TORSION_COSINE)
def convert_torsion_potential(
    pot: fitlib.TensorPotential, param_map: fitlib.ValenceParameterMap, types: list[str]
):
    is_proper = pot.type == fitlib.PotentialType.PROPER_TORSIONS
...
def _convert_to_openmm_ffxml(
    ...
    omm_top = fitlib.converters.convert_to_openmm_topology(top)
    ...
    for atom in omm_top.atoms():
        ...
        atom_name_to_type[atom.name] = f"fitlib-{short_uuid}-{atom_type}"
...
def convert_to_openmm_ffxml(
    ...
    system: fitlib.TensorSystem | fitlib.TensorTopology,
) -> list[str]:
    """Convert a fitlib force field and system to OpenMM force field XML
    representations.
    ...
    """
    if isinstance(system, fitlib.TensorTopology):
        system = fitlib.TensorSystem([system], [1], False)
