...
_CONVERTER_FUNCTIONS: dict[
    tuple[str, str],
    typing.Callable[
        [fitlib.TensorPotential, fitlib.TensorSystem], openmm.Force | list[openmm.Force]
    ],
] = {}
...
def create_openmm_system(
    system: fitlib.TensorSystem, v_sites: fitlib.TensorVSites | None
) -> openmm.System:
    """Create an empty OpenMM system from a ``fitlib`` system."""
    ...
    for topology, n_copies in zip(system.topologies, system.n_copies, strict=True):
        for _ in range(n_copies):
            ...
            for key, parameter_idx in zip(
                ...
                local_frame_coords = fitlib.geometry.polar_to_cartesian_coords(
...
def _apply_constraints(omm_system: openmm.System, system: fitlib.TensorSystem):
    idx_offset = 0
...
def convert_to_openmm_force(
    ...
    """Convert a ``fitlib`` potential to OpenMM forces.
    ...
    importlib.import_module("fitlib.converters.openmm.nonbonded")
    importlib.import_module("fitlib.converters.openmm.valence")
...
def convert_to_openmm_system(
    ...
    """Convert a ``fitlib`` force field and system / topology into an OpenMM system.
...
def convert_to_openmm_topology(
    ...
    """Convert a ``fitlib`` system to an OpenMM topology.
