...
def _create_nonbonded_force(
    ...
    cutoff_idx = potential.attribute_cols.index(fitlib.CUTOFF_ATTRIBUTE)
    switch_idx = (
        None
        if fitlib.SWITCH_ATTRIBUTE not in potential.attribute_cols
        else potential.attribute_cols.index(fitlib.SWITCH_ATTRIBUTE)
    )
...
def _detect_parameters(
    potential: fitlib.TensorPotential, energy_fn: str, mixing_fn: dict[str, str]
...
def _extract_parameters(
    potential: fitlib.TensorPotential, parameter: torch.Tensor, cols: list[str]
) -> list[float]:
    ...
    for topology, n_copies in zip(system.topologies, system.n_copies, strict=True):
        ...
        for _ in range(n_copies):
            ...
            for index, (i, j) in enumerate(parameter_map.exclusions):
                ...
                if torch.isclose(scale, fitlib.utils.tensor_like(0.0, scale)):
                    continue
...
@fitlib.converters.openmm.potential_converter(
    fitlib.PotentialType.VDW, fitlib.EnergyFn.VDW_LJ
)
    ...
    for topology, n_copies in zip(system.topologies, system.n_copies, strict=True):
        ...
        for _ in range(n_copies):
            ...
            for index, (i, j) in enumerate(parameter_map.exclusions):
                ...
                eps, sig = fitlib.potentials.nonbonded.lorentz_berthelot(
