...
def _convert_v_sites(
...
) -> tuple[fitlib.TensorVSites, list[fitlib.VSiteMap | None]]:
    ...
    parameters = [
        [
            _get_value(
                parameters_by_key[parameter_key],
                column,
                fitlib.TensorVSites.default_units(),
    ...
    v_site_frames = [
        fitlib.geometry.V_SITE_TYPE_TO_FRAME[parameter_key_to_type[parameter_key]]
        for parameter_key in parameter_keys
    ]
    v_sites = fitlib.TensorVSites(
...
def _convert_constraints(
...
) -> list[fitlib.TensorConstraints | None]:
    handler_types = {handler.type for handler in handlers}
    ...
    for handler in handlers:
        ...
        constraint = fitlib.TensorConstraints(
...
def convert_handlers(
    handlers: list[openff.interchange.smirnoff.SMIRNOFFCollection],
    topologies: list[openff.toolkit.Topology],
    v_site_maps: list[fitlib.VSiteMap | None] | None = None,
    potentials: (
        list[tuple[fitlib.TensorPotential, list[fitlib.ParameterMap]]] | None
    ) = None,
    constraints: list[fitlib.TensorConstraints | None] | None = None,
) -> list[tuple[fitlib.TensorPotential, list[fitlib.ParameterMap]]]:
    ...
    """
    importlib.import_module("fitlib.converters.openff.nonbonded")
    importlib.import_module("fitlib.converters.openff.valence")
...
def _convert_topology(
    ...
    parameters: dict[str, fitlib.ParameterMap],
    v_sites: fitlib.VSiteMap | None,
    constraints: fitlib.TensorConstraints | None,
) -> fitlib.TensorTopology:
...
def convert_interchange(
...
) -> tuple[fitlib.TensorForceField, list[fitlib.TensorTopology]]:
    ...
    tensor_force_field = fitlib.TensorForceField(potentials, v_sites)
