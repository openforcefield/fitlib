...
def test_apply_hmr():
    ...
    force_field, [topology_water, topology_meoh] = (
        fitlib.converters.convert_interchange(interchanges)
    )
    ...
    system_openmm = fitlib.converters.convert_to_openmm_system(
        force_field, system_fitlib
    )
    ...
    _apply_hmr(system_openmm, system_fitlib)
...
def test_topology_to_rdkit():
    ...
    topology = fitlib.TensorTopology(
...
def test_topology_to_xyz(mocker):
    ...
    mocker.patch("fitlib.mm._utils.topology_to_rdkit", return_value=mock_molecule)
...
def test_generate_system_coords():
    coords, box_vectors = generate_system_coords(
        ...
        None,
        fitlib.mm.GenerateCoordsConfig(),
    )
...
def test_generate_system_coords_with_v_sites():
    ...
    force_field, [topology] = fitlib.converters.convert_interchange(interchange)
    system = fitlib.TensorSystem([topology], [1], False)
...
def test_simulate(mocker, mock_argon_tensors):
    ...
    spied_energy_minimize = mocker.spy(fitlib.mm._mm, "_energy_minimize")
    spied_run_simulation = mocker.spy(fitlib.mm._mm, "_run_simulation")
