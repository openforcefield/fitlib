...
def test_compute_energy(precision, smiles: str):
    ...
    energy_fitlib = compute_energy(tensor_top, tensor_ff, conformer, None)
    energy_openmm = _compute_openmm_energy(interchange, conformer)

    assert torch.isclose(energy_fitlib, energy_openmm.to(energy_fitlib.dtype))
...
def test_compute_energy_periodic(etoh_water_system, precision):
    ...
    energy_fitlib = compute_energy(tensor_sys, tensor_ff, coords, box_vectors)

    openmm_system = fitlib.converters.openmm.convert_to_openmm_system(
    ...
    assert torch.isclose(
        energy_fitlib, torch.tensor(energy_openmm, dtype=energy_fitlib.dtype)
    )
...
def test_compute_energy_v_sites():
    ...
    energy_openmm = _compute_openmm_energy(interchange, conformer)
    energy_fitlib = compute_energy(tensor_top, tensor_ff, conformer)
...
def test_energy_backward_pass(periodic):
    ...
    energy = fitlib.compute_energy(tensor_sys, tensor_ff, coords, box_vectors)
    energy.backward()
