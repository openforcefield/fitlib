...
def predict(
    ...
    for entry in dataset:
        ...
        if box_vectors is not None:
            # fitlib does not support batched periodic evaluations,
            # so we loop over conformers.
            box_vectors = fitlib.utils.tensor_like(box_vectors, coords_flat).reshape(
                len(energy_ref), 3, 3
            )
            energy_pred = torch.cat(
                [
                    fitlib.compute_energy(
            ...
            energy_pred = fitlib.compute_energy(topology, force_field, coords, None)
    ...
    energy_ref_all = fitlib.utils.tensor_like(energy_ref_all, energy_pred_all)

    forces_ref_all = torch.cat(forces_ref_all)
    forces_ref_all = fitlib.utils.tensor_like(forces_ref_all, forces_pred_all)
