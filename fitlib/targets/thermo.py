...
import fitlib.utils.loss
import fitlib.utils.molecule
...
class SimulationConfig(pydantic.BaseModel):
    ...
    gen_coords: fitlib.mm.GenerateCoordsConfig = pydantic.Field(
    ...
    equilibrate: list[fitlib.mm.MinimizationConfig | fitlib.mm.SimulationConfig] = (
        pydantic.Field(..., description="Configuration for equilibration simulations.")
    )

    production: fitlib.mm.SimulationConfig = pydantic.Field(
...
_SystemDict = dict[SimulationKey, fitlib.TensorSystem]
...
def create_dataset(*rows: DataEntry) -> datasets.Dataset:
    ...
    for row in rows:
        row["smiles_a"] = fitlib.utils.molecule.map_smiles(row["smiles_a"])

        if row["smiles_b"] is None:
            continue

        row["smiles_b"] = fitlib.utils.molecule.map_smiles(row["smiles_b"])
...
def _convert_entry_to_system(
    entry: DataEntry, topologies: dict[str, fitlib.TensorTopology], max_mols: int
) -> tuple[SimulationKey, fitlib.TensorSystem]:
    ...
    system_topologies: list[fitlib.TensorTopology] = [topologies[smiles_a]]
    n_copies = [n_copies_a]
    ...
    system = fitlib.TensorSystem(system_topologies, n_copies, True)
...
def _bulk_config(temperature: float, pressure: float) -> SimulationConfig:
    ...
    return SimulationConfig(
        max_mols=256,
        gen_coords=fitlib.mm.GenerateCoordsConfig(),
        equilibrate=[
            fitlib.mm.MinimizationConfig(),
            # short NVT equilibration simulation
            fitlib.mm.SimulationConfig(
        ...
        production=fitlib.mm.SimulationConfig(
            temperature=temperature,
            pressure=pressure,
            n_steps=500000,
...
def select_config(
    ...
    try:
        ...
        for stage in config.equilibrate:
            if isinstance(stage, fitlib.mm.SimulationConfig):
...
def _plan_simulations(
    ...
    topologies: dict[str, fitlib.TensorTopology],
    ...
    for entry in entries:
        ...
        if _REQUIRES_VACUUM_SIM[data_type]:
            ...
            system = fitlib.TensorSystem([topologies[entry["smiles_a"]]], [1], False)
...
def _simulate(
    ...
    """
    coords, box_vectors = fitlib.mm.generate_system_coords(
    ...
    with output_path.open("wb") as output:
        reporter = fitlib.mm.TensorReporter(
...
def _compute_observables(
    ...
    if cached_path is not None and cached_path.exists():
        with contextlib.suppress(fitlib.mm.NotEnoughSamplesError):
            means = fitlib.mm.reweight_ensemble_averages(
                system, force_field, cached_path, temperature, pressure
            )
            stds = {
                key: fitlib.utils.tensor_like(torch.nan, means[key]) for key in means
            }
    ...
    return _Observables(
        *fitlib.mm.compute_ensemble_averages(
...
def _predict_hvap(
    ...
    system_bulk: fitlib.TensorSystem,
...
def _predict_hmix(
    ...
    system_mix: fitlib.TensorSystem,
    system_0: fitlib.TensorSystem,
    system_1: fitlib.TensorSystem,
) -> tuple[torch.Tensor, torch.Tensor | None]:
    assert entry["units"] == "kcal/mol"
...
def _predict(
    ...
    systems: dict[Phase, dict[SimulationKey, fitlib.TensorSystem]],
) -> tuple[torch.Tensor, torch.Tensor | None]:
...
def predict(
            ...
            verbose_rows.append(
                {
                    ...
                    "smiles_a": fitlib.utils.molecule.unmap_smiles(entry["smiles_a"]),
                    "smiles_b": (
                        ""
                        if entry["smiles_b"] is None
                        else fitlib.utils.molecule.unmap_smiles(entry["smiles_b"])
                    ),
    ...
    reference = fitlib.utils.tensor_like(reference, predicted)
    reference_std = fitlib.utils.tensor_like(reference_std, predicted_std)

    return reference, reference_std, predicted, predicted_std


def default_closure(
    trainable: "fitlib.train.Trainable",
    topologies: dict[str, fitlib.TensorTopology],
    dataset: datasets.Dataset,
    per_type_scales: dict[DataType, float] | None = None,
    verbose: bool = False,
    simulation_config: dict[str, SimulationConfig] | None = None,
) -> fitlib.optim.ClosureFn:
    ...
    def closure_fn(
        ...
        if compute_hessian:
            hessian = fitlib.utils.loss.approximate_hessian(x, y_pred)
        if compute_gradient:
