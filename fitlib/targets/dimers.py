...
def create_from_des(
    ...
    for system_id in tqdm.tqdm(system_ids, desc="loading dimers"):
        ...
        for group_id in group_ids:
            ...
            smiles_a = fitlib.utils.molecule.mol_to_smiles(mol_a, False)
            smiles_b = fitlib.utils.molecule.mol_to_smiles(mol_b, False)
...
def compute_dimer_energy(
    topology_a: fitlib.TensorTopology,
    topology_b: fitlib.TensorTopology,
    ...
    """
    dimer = fitlib.TensorSystem([topology_a, topology_b], [1, 1], False)

    coords_a = coords[:, : topology_a.n_atoms, :]

    if topology_a.v_sites is not None:
        coords_a = fitlib.geometry.add_v_site_coords(
    ...
    if topology_b.v_sites is not None:
        coords_b = fitlib.geometry.add_v_site_coords(
            topology_b.v_sites, coords_b, force_field
        )

    coords = torch.cat([coords_a, coords_b], dim=1)

    energy_dimer = fitlib.compute_energy(dimer, force_field, coords)

    energy_a = fitlib.compute_energy(topology_a, force_field, coords_a)
    energy_b = fitlib.compute_energy(topology_b, force_field, coords_b)
...
def _predict(
    ...
    coords_flat = fitlib.utils.tensor_like(
    ...
    reference = fitlib.utils.tensor_like(dimer["energy"], predicted)
...
def predict(
    ...
    reference, predicted = zip(
        *[
            ...
            for dimer in fitlib.utils.dataset.iter_dataset(dataset)
        ],
        strict=True,
    )
...
def default_closure(
    ...
    return fitlib.utils.loss.to_closure(loss_fn)
...
def _plot_energies(energies: dict[str, torch.Tensor]) -> str:
    ...
    for i, (k, v) in enumerate(energies.items()):
        axis.plot(
            ...
            label=k,
            linestyle="none",
            marker=fitlib.utils.reporting.DEFAULT_MARKERS[i],
            color=fitlib.utils.reporting.DEFAULT_COLORS[i],
        )
    ...
    figure.tight_layout()
    img = fitlib.utils.reporting.figure_to_img(figure)
...
def report(
    dataset: datasets.Dataset,
    force_fields: dict[str, fitlib.TensorForceField],
    topologies: dict[str, dict[str, fitlib.TensorTopology]],
    ...
    for dimer in fitlib.utils.dataset.iter_dataset(dataset):
        ...
        mol_img = fitlib.utils.reporting.mols_to_img(
