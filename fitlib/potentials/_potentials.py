...
if typing.TYPE_CHECKING:
    import fitlib.potentials.nonbonded
...
def broadcast_exceptions(
    ...
    for topology, n_copies in zip(system.topologies, system.n_copies, strict=True):
        ...
        if isinstance(parameter_map, fitlib.ValenceParameterMap):
...
def broadcast_idxs(
    ...
    for topology, n_copies in zip(system.topologies, system.n_copies, strict=True):
        ...
        offset = (
            idx_offset + fitlib.utils.arange_like(n_copies, idxs) * topology.n_particles
        )
...
) -> tuple[fitlib.TensorSystem, torch.Tensor, torch.Tensor | None]:
...
def _precompute_pairwise(
    ...
    conformer: torch.Tensor,
    box_vectors: torch.Tensor | None = None,
) -> typing.Optional["fitlib.potentials.nonbonded.PairwiseDistances"]:
    ...
    for potential in force_field.potentials:
        ...
        if fitlib.CUTOFF_ATTRIBUTE in potential.attribute_cols:
    ...
    return fitlib.potentials.nonbonded.compute_pairwise(
...
def compute_energy_potential(
    ...
    conformer: torch.Tensor,
    box_vectors: torch.Tensor | None = None,
    pairwise: typing.Optional["fitlib.potentials.nonbonded.PairwiseDistances"] = None,
) -> torch.Tensor:
    ...
    importlib.import_module("fitlib.potentials.nonbonded")
    importlib.import_module("fitlib.potentials.valence")
...
def compute_energy(
    ...
    energy = fitlib.utils.zeros_like(
