...
def _broadcast_exclusions(
    system: fitlib.TensorSystem, potential: fitlib.TensorPotential
    ...
    for topology, n_copies in zip(system.topologies, system.n_copies, strict=True):
        ...
        exclusion_offset = (
            idx_offset
            + fitlib.utils.arange_like(n_copies, exclusion_idxs) * topology.n_particles
        )
...
def compute_pairwise_scales(
    ...
    pair_scales = fitlib.utils.ones_like(n_pairs, other=potential.parameters)

    if len(exclusion_idxs) > 0:
        exclusion_idxs, _ = exclusion_idxs.sort(dim=1)  # ensure upper triangle

        pair_idxs = fitlib.utils.to_upper_tri_idx(
...
def prepare_lrc_types(
    system: fitlib.TensorSystem,
    potential: fitlib.TensorPotential,
    ...
    counts = fitlib.utils.tensor_like(
...
def lorentz_berthelot(
    ...
    Returns:
        ...
        ``shape=(n_pairs, 1)``.
    """
    return fitlib.utils.geometric_mean(epsilon_a, epsilon_b), 0.5 * (sigma_a + sigma_b)
...
def _compute_switch_fn(
    ...
    if fitlib.SWITCH_ATTRIBUTE not in potential.attribute_cols:
        return torch.ones(1), None

    switch_width_idx = potential.attribute_cols.index(fitlib.SWITCH_ATTRIBUTE)
...
def _integrate_lj_switch(
    ...
    coeff_0 = fitlib.utils.tensor_like([rs**3, rs**2, rs, 1, b, b**2], rs) * (
        fitlib.utils.tensor_like([rs**2 * b**2, rs * b, 1], rs)
        * fitlib.utils.tensor_like(
    ...
    coeff_01 = (
        sig[:, None] ** 6
        * fitlib.utils.tensor_like([-28, 945, -1080, 420, -756, 378], rs)
        * coeff_0
    )
    coeff_11 = (
        fitlib.utils.tensor_like([84, -3780, 7560, 2520, -3780, 756], rs) * coeff_0
    )

    powers = fitlib.utils.tensor_like([-9, -8, -7, -6, -5, -4, -3, -2, -1, 0, 1, 2], rs)
...
def _compute_lj_lrc(
    ...
    idxs_i, idxs_j, n_ij_interactions = fitlib.potentials.nonbonded.prepare_lrc_types(
        system, potential
    )
    ...
    eps_ij, sig_ij = fitlib.potentials.nonbonded.lorentz_berthelot(
...
@fitlib.potentials.potential_energy_fn(fitlib.PotentialType.VDW, fitlib.EnergyFn.VDW_LJ)
def compute_lj_energy(
    ...
    cutoff = potential.attributes[
        potential.attribute_cols.index(fitlib.CUTOFF_ATTRIBUTE)
    ]
    ...
    parameters = fitlib.potentials.broadcast_parameters(system, potential)
    pair_scales = compute_pairwise_scales(system, potential)

    pairs_1d = fitlib.utils.to_upper_tri_idx(
    ...
    if potential.exceptions is not None:
        exception_idxs, exceptions = fitlib.potentials.broadcast_exceptions(
...
def _integrate_dexp_switch(
    ...
    """
    rs_pow = fitlib.utils.tensor_like(
        [rs**5, rs**4, rs**3, rs**2, rs, 1, 0, 0], rc
    ).unsqueeze(1)

    # fmt: off
    c_n = fitlib.utils.tensor_like(
        [
    ...
    b = b.unsqueeze(1)
    b_pow = torch.hstack(
        [fitlib.utils.zeros_like((len(b), 1), b)] * 4
    ...
    # fmt: off
    mat_coeff = fitlib.utils.tensor_like(
        [
...
def _compute_dexp_lrc(
    ...
    eps_ij, r_min_ij = fitlib.potentials.nonbonded.lorentz_berthelot(
...
@fitlib.potentials.potential_energy_fn(
    fitlib.PotentialType.VDW, fitlib.EnergyFn.VDW_DEXP
)
def compute_dexp_energy(
    ...
    eps, r_min = fitlib.potentials.nonbonded.lorentz_berthelot(
        parameters[pairwise.idxs[:, 0], eps_column],
        parameters[pairwise.idxs[:, 1], eps_column],
...
def _compute_coulomb_energy_periodic(
    ...
    charges = fitlib.potentials.broadcast_parameters(system, potential).squeeze(-1)
...
@fitlib.potentials.potential_energy_fn(
    fitlib.PotentialType.ELECTROSTATICS, fitlib.EnergyFn.COULOMB
)
