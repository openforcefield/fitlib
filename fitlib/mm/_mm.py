...
import fitlib.mm._config
import fitlib.mm._reporters
import fitlib.mm._utils

_LOGGER = logging.getLogger("fitlib.mm")
...
def _apply_hmr(
    ...
    system_fitlib: fitlib.TensorSystem,
    ...
    Args:
        ...
        system_fitlib: The fitlib system that defines the system topology.
    ...
    for topology, n_copies in zip(
        system_fitlib.topologies, system_fitlib.n_copies, strict=True
    ):
...
def _topology_to_xyz(
    topology: fitlib.TensorTopology, force_field: fitlib.TensorForceField | None
) -> str:
    """Convert a topology to an RDKit molecule."""
    mol = fitlib.mm._utils.topology_to_rdkit(topology)
    ...
    if topology.n_v_sites > 0:
        ...
        coords = fitlib.geometry.add_v_site_coords(
...
def _approximate_box_size(
    ...
    config: "fitlib.mm.GenerateCoordsConfig",
...
def generate_system_coords(
    system: fitlib.TensorSystem,
    force_field: fitlib.TensorForceField | None,
    config: typing.Optional["fitlib.mm.GenerateCoordsConfig"] = None,
    ...
    config = config if config is not None else fitlib.mm.GenerateCoordsConfig()
...
def _energy_minimize(
    ...
    config: "fitlib.mm.MinimizationConfig",
) -> openmm.State:
...
def _run_simulation(
    ...
    state: openmm.State | tuple[openmm.unit.Quantity, openmm.unit.Quantity],
    platform: openmm.Platform,
    config: "fitlib.mm.SimulationConfig",
...
def simulate(
    ...
    equilibrate_configs: list[
        typing.Union["fitlib.mm.MinimizationConfig", "fitlib.mm.SimulationConfig"]
    ],
    production_config: "fitlib.mm.SimulationConfig",
    production_reporters: list[typing.Any] | None = None,
    apply_hmr: bool = False,
) -> openmm.State:
    """Simulate a fitlib system of molecules or topology.
    ...
    system: fitlib.TensorSystem = (
        system
        if isinstance(system, fitlib.TensorSystem)
        else fitlib.TensorSystem([system], [1], False)
    ).to("cpu")

    requires_pbc = any(
        config.pressure is not None
        for config in equilibrate_configs + [production_config]
        if isinstance(config, fitlib.mm.SimulationConfig)
    )
    ...
    omm_system = fitlib.converters.convert_to_openmm_system(force_field, system)
    omm_topology = fitlib.converters.convert_to_openmm_topology(system)
    ...
    for i, config in enumerate(equilibrate_configs):
        ...
        if isinstance(config, fitlib.mm.MinimizationConfig):
            omm_state = _energy_minimize(omm_system, omm_state, platform, config)

        elif isinstance(config, fitlib.mm.SimulationConfig):
