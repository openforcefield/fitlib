...
def _extract_pure_solvent(
    solute: fitlib.TensorTopology,
    solvent: fitlib.TensorTopology | None,
    ...
    system = fitlib.TensorSystem([solvent], [system.n_copies[1]], is_periodic=True)
...
def _to_solvent_dict(
    solvent: fitlib.TensorTopology, n_solvent: int
) -> dict[str, int] | None:
    if solvent is None:
        return None

    smiles = openff.toolkit.Molecule.from_rdkit(
        fitlib.mm._utils.topology_to_rdkit(solvent),
...
def generate_dg_solv_data(
    solute: fitlib.TensorTopology,
    solvent_a: fitlib.TensorTopology | None,
    solvent_b: fitlib.TensorTopology | None,
    force_field: fitlib.TensorForceField,
    ...
    solute_mol = openff.toolkit.Molecule.from_rdkit(
        fitlib.mm._utils.topology_to_rdkit(solute),
        allow_undefined_stereo=True,
    )
    ...
    topologies = {
        "solvent-a": fitlib.TensorSystem([solute], [1], is_periodic=False)
        if solvent_a is None
        else fitlib.TensorSystem(
            [solute, solvent_a], [1, n_solvent_a], is_periodic=True
        ),
        "solvent-b": fitlib.TensorSystem([solute], [1], is_periodic=False)
        if solvent_b is None
        else fitlib.TensorSystem(
            [solute, solvent_b], [1, n_solvent_b], is_periodic=True
        ),
    }
    pressures = {
        "solvent-a": None
    ...
    def _parameterize(
        ...
        return fitlib.converters.convert_to_openmm_system(
            force_field, topologies[phase]
        )
...
def _load_trajectory(
    ...
    topology_omm = fitlib.converters.convert_to_openmm_topology(system)
...
def _compute_energy(
    ...
    if system.is_periodic:
        energy_per_frame = [
            fitlib.compute_energy(system, ff, c, b)
            for c, b in zip(xyz_0, box_0, strict=True)
        ]
        energy = torch.concat(energy_per_frame)
    else:
        energy = fitlib.compute_energy(system, ff, xyz_0, box_0)

    return energy


def _compute_grads_solvent(
    solvent: fitlib.TensorTopology,
    force_field: fitlib.TensorForceField,
    ...
    system = fitlib.TensorSystem([solvent], [n_solvent], is_periodic=True).to(device)
...
def compute_dg_and_grads(
    ...
    dg = (f_i[-1] - f_i[0]) / beta
    dg = fitlib.utils.tensor_like(dg, force_field.potentials[0].parameters)
...
def _reweight_dg_and_grads(
    ...
    with torch.enable_grad():
        ...
        weights = fitlib.utils.tensor_like(mbar.W_nk[:, 1], energy_new)
        grads = ()
...
def reweight_dg_and_grads(
    ...
    system_solv = fitlib.TensorSystem([solvent], [n_solvent], is_periodic=True).to(
        device
    )
    ...
    n_0_solv = fitlib.utils.tensor_like([len(u_0_solv)], u_0_solv)
