...
def _pack_force_field(
    ...
    """Pack a fitlib force field into a tuple that can be consumed by a custom PyTorch op.
...
def _unpack_force_field(
    ...
    has_v_sites: bool,
    force_field: fitlib.TensorForceField,
) -> fitlib.TensorForceField:
    """Unpack a fitlib force field from its packed tensor and lookup representation.
    ...
    return fitlib.TensorForceField(potentials, v_sites)


def _compute_mass(system: fitlib.TensorSystem) -> float:
...
def _compute_observables(
    ...
    for coords, box_vectors, _, kinetic in fitlib.mm._reporters.unpack_frames(
        frames_file
    ):
        ...
        with torch.enable_grad():
            potential = fitlib.compute_energy(system, force_field, coords, box_vectors)
        ...
        if len(needs_grad) > 0:
            du_d_theta_subset = torch.autograd.grad(
                ...
                [fitlib.utils.ones_like(1, potential)],
                retain_graph=False,
                allow_unused=True,
            )
    ...
    reduced_potentials = fitlib.utils.tensor_like(reduced_potentials, theta[0])
...
class _ReweightAverageOp(torch.autograd.Function):
    ...
    def forward(ctx, kwargs: _ReweightAverageKwargs, *theta: torch.Tensor):
        ...
        with kwargs["frames_path"].open("rb") as file:
            reduced_pot_0 = fitlib.utils.tensor_like(
                [v for _, _, v, _ in fitlib.mm._reporters.unpack_frames(file)],
                reduced_pot,
            )
    ...
    def backward(ctx, *grad_outputs):
        ...
        for i in range(len(du_d_theta)):
            ...
            log_exp_sum, log_exp_sign = fitlib.utils.logsumexp(
...
class _ComputeDGSolv(torch.autograd.Function):
    ...
    def forward(ctx, kwargs, *theta: torch.Tensor):
        from fitlib.mm._fe import compute_dg_and_grads
...
class _ReweightDGSolv(torch.autograd.Function):
    @staticmethod
    def forward(ctx, kwargs, *theta: torch.Tensor):
        from fitlib.mm._fe import reweight_dg_and_grads
